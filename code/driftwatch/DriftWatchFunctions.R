...
plotAPIDrift <- function(drift, etopo = 'etopo180.nc', filename=NULL, bathy=TRUE, sl=TRUE, wca=TRUE,
    ...
    for(d in seq_along(unique(drift[[labelBy]]))) {
      ...
      # Skip if there's only one data point for the drift
      if(nrow(thisDrift) == 1) next
      
      # Find the start and end points based on UTC time
      start <- which.min(thisDrift$UTC)
      thisDrift <- arrange(thisDrift, UTC)  # Order the data by UTC
      
      # Plot the drift lines (main path and a grey line for the full drift)
      lines(x=thisDrift$Longitude, y=thisDrift$Latitude, col='black', lwd=2)
      lines(x=thisDrift$Longitude, y=thisDrift$Latitude, col='grey', lwd=1, lty=5)
      
      # If not in simple mode, label the start and end points with numbers
      if(isFALSE(simple)) {
        # Plot start and end markers
        points(x=thisDrift$Longitude[start], y=thisDrift$Latitude[start], pch=15, cex=1.2)  # Start marker
        end <- which.max(thisDrift$UTC)  # End marker
        points(x=thisDrift$Longitude[end], y=thisDrift$Latitude[end], pch=17, col='grey', cex=1.2)  # End marker
        
        # Add numeric labels at the start and end points (using mapLabs[d] for each drift)
        text(x=thisDrift$Longitude[start], y=thisDrift$Latitude[start], labels=mapLabs[d], col='white', cex=0.3)
        text(x=thisDrift$Longitude[end], y=thisDrift$Latitude[end], labels=mapLabs[d], col='black', cex=0.3)
      }
    }
    
    # If in simple mode, either return the drift data or the filename (based on the function requirements)
    if(isTRUE(simple)) {
      if(is.null(filename)) {
        return(invisible(drift))  # Return drift data invisibly if no filename
      }
      return(invisible(filename))  # Return filename invisibly if given
    }
    
    # Add the legend with numeric labels (1 to N)
    lastLegend <- legend(x='topright', 
                         legend=c('Start', 'End', unique(drift[[labelBy]])), 
                         col=c('black', 'grey', rep('black', length(mapLabs))), 
                         pch=c(15, 17, rep(16, length(mapLabs))),  # Use circle pch for numeric labels
                         merge=FALSE, 
                         seg.len = 1, 
                         cex=1, 
                         plot=FALSE)
    
    useCex <- min(.2 * diff(xlim) / lastLegend$rect$w, 1)  # Adjust font size based on plot size
    
    # Update the legend with the numeric labels (1 to N)
    lastLegend <- legend(x='topright', 
                         legend=c('Start', 'End', paste("Drift", mapLabs)),  # Add numeric drift labels
                         col=c('black', 'grey', rep('black', length(mapLabs))), 
                         pch=c(15, 17, rep(16, length(mapLabs))),  # Circle pch for numeric labels
                         merge=FALSE, 
                         seg.len = 1, 
                         cex=useCex)
    
    # Add gradient legends (if applicable)
    if(current) {
      lastLegend <- myGradLegend(lastLeg=lastLegend, vals=curLeg$vals, cols=curLeg$colors,
                                 title='Current (Knots)', cex=useCex, tCex=.67)
    }
    ...
    # If a filename is specified, return the filename; otherwise, return the drift data
    if(is.null(filename)) {
      return(invisible(drift))
    }
    invisible(filename)
